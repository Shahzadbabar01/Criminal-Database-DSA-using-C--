...
int WrongActs :: deleteData(string deleteId)
{
    if(!isEmpty())
    {
     if(head -> id == deleteId)
        ...
        {
        WrongActNode *previous;
        WrongActNode *current;
        current = head;
...
void WrongActs :: addData(string id1, string date1, string name1, string phone_no1, string address1, string crime1, string charges1, string criminalDetails1)
{
            WrongActNode *temp = new WrongActNode;
            temp -> id = id1;
            temp -> date = date1;
            temp -> name = name1;
...
void WrongActs :: saveData()
{
      ofstream myFile("WrongActCriminals.txt" , ios::app);
           WrongActNode *temp;
           temp = head;
           while(temp != NULL)
           {
...
int WrongActs :: countFileLines()
{
    string asd;
    int countLines = 0;
    ifstream myFile("WrongActCriminals.txt");
    if(myFile.is_open())
    {
...
void WrongActs :: clearFile()
{
    ofstream File("WrongActCriminals.txt", ios::trunc);
    File.close();
}

bool WrongActs :: searchData(string key)
{
     if(!isEmpty())
    {
        WrongActNode *temp;
...
void WrongActs :: readFile()
{
    ifstream File("WrongActCriminals.txt");
    if(File.is_open())
    {
         int i = 0;
    int j = 1;
...
void WrongActs :: updateData(string upDateId)
{
     if(!isEmpty())
    {
    WrongActNode *temp;
